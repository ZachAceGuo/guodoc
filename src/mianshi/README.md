...
- **服务启动时就会注册**自己的服务信息（服务名、IP、端口）到注册中心
...
- 当注册中心**长时间收不到提供者的心跳**时，会认为这个实例宕机，将其从服务的实例列表中剔除
- 当服务**有新实例启动时**，会**发送注册服务请求**，其信息**会被记录在注册中心的服务实例列表**
- 当**注册中心服务列表变更时**，**会主动通知微服务，更新本地服务列表**
...
- 服务实例：启动后向 Nacos 注册自己，并周期性**每隔5秒**发送心跳信息到nacos。
  - **心跳信息是使用http的put请求**发送，包括：服务名称、IP、端口、集群名称、权重、是否为临时实例等
- Nacos 服务端：接收心跳，如果 Nacos 在 **15 秒**内（也就是3 次心跳）没有收到心跳，将实**例标记为 不健康**。如果连续 **30 秒**（6 次心跳）**未收到心**跳，Nacos 将该**实例从服务列表中 删除**（Deregister）
...
在网关怎么路由的； 回答配置 `routes `节点下配置 `id`、`uri`、`predicates` 路由断言：
...
- **TM：事务管理器** —— 负责发起、提交或回滚全局事务
- **RM：资源管理器** —— 负责执行业务逻辑和本地数据库操作
- **TC：事务协调器** —— 全局仲裁中心，决定事务是提交还是回滚
...
> 当一个业务方法标了 `@GlobalTransactional` 时，TM（事务管理器）就会向 TC（事务协调器）**注册一个全局事务**，生成一个全局事务 ID，也就是 XID。后续所有分支事务都在这个 XID 之下运行。

在各业务服务执行本地事务的时候，它们作为 RM（资源管理器）执行 SQL 时，Seata 的 AT 模式会**拦截 SQL 执行前和执行后的数据快照**，也就是**前镜像和后镜像**。
 这意味着局部事务在业务代码层面其实已经提交了，但 Seata 底层记录了快照，所以**它有能力回滚**。

业务执行完后：

- 各 RM 会把执行结果（包括快照）上报给 TC
- TM 发起全局事务的提交或回滚请求
- TC 根据所有 RM 的状态决定最终走提交还是回滚
...
> XA 模式是数据库层面的两阶段，**本地事务不会立即提交**。

流程和 AT 开头一样：
 TM 注册全局事务、各 RM 执行业务。

但 RM 执行完 SQL 后不会直接提交，而是进入**准备状态**等待 TC 的指令。

- 如果 TC 决定 commit，那么 RM 才真正提交事务
- 如果 TC 决定 rollback，那么 RM 执行数据库级回滚
...
**两阶段提交（2PC）与 Seata 的关系**

无论 AT 还是 XA，本质都实现了两阶段提交协议：
...
- 事务协调器 汇总所有 资源管理器 的状态
- 一致就发 commit
- 有异常就发 rollback

一句话概括：
 **第一阶段问大家“准备好了吗？”，第二阶段决定“上还是撤”。**
...
第一类是应用运行基础注解：
 `@SpringBootApplication`，用于标记一个 Spring Boot 主程序入口，触发自动配置。

第二类是容错能力
 `@HystrixCommand`，我们在方法上标记它表示该方法具有熔断能力，当调用失败时可以走降级逻辑。

第三类是配置绑定：
 `@ConfigurationProperties` 可以把配置文件中的属性和 Java Bean 自动绑定，避免硬编码。

第四类是微服务调用：
 `@EnableFeignClients` 开启 Feign 声明式调用支持，允许我们用接口调用远程服务。

第五类是消息驱动：
 `@StreamListener` 结合 Spring Cloud Stream 监听消息中间件的数据流，实现消息消费逻辑。

其余像 `@RequestMapping`（接口路由）和 `@MapperScan`（扫描 MyBatis Mapper）属于常规编程基础注解。
...
微服务架构中，只要涉及**跨服务的内部调用**，我们就优先使用 Feign

我们有一个公共服务叫 publics，里面封装了微信接口、短信接口、高德地图接口等，其他服务要用它，不会自己发 HTTP 请求，而是通过 Feign 调用。

当用户下单或取消订单时，我们服务需要调用营销服务核销或退回优惠券，也用 Feign 来完成。

只要是微服务之间交互、避免硬编码 HTTP 客户端，我们使用 Feign。

===============

在我们的项目中，只要是跨微服务的服务调用都使用到了；比如说：

- 我们有一个公共的publics服务，提供了高德地图的接口、微信公众平台的接口、短信接口等等；其它服务需要时就通过OpenFeign进行调用
- 用户下单或取消订单时，也需要通过OpenFeign调用营销服务进行 优惠券的核销或退还

#### OpenFeign原理

Feign 的设计理念是：**把 HTTP 请求当成方法调用。**

执行机制：

定义一个接口，并用 `@FeignClient` 标记它，指定要调用的目标服务名称。@RequestMapping等注解指定HTTP方法和路径

项目启动时，Feign 会根据这个接口生成一个动态代理对象。

代理会根据服务名，从本地缓存里找可用实例，负载均衡（默认轮询）挑一个服务器地址

再把我们写的方法映射成实际 HTTP URL，用 HttpClient 或 OkHttp 去发请求。

如果本地没有缓存实例，它会访问注册中心拉取服务实例，再缓存起来。

Feign 代理帮我们干了三件事：
 **服务发现 → 路由选择 → HTTP 调用包装成方法调用。**
...
它是一种**保护机制**。
 当某服务失败率达到阈值时，系统会中断对它的调用，防止继续拖垮系统。

闭合状态（Closed）—— 服务正常

断开状态（Open）—— 达到阈值，切断调用

半开状态（Half-Open）—— 让少量请求试探恢复
...
当访问失败或熔断触发时，我们不报错崩溃，而是提供一个备选策略。
...
熔断侧重“断开保护”，降级侧重“兜底响应”，这两者组合才能真正保证系统韧性。
...
- 闭合状态（Closed）：服务正常时，熔断器是闭合的，正常请求可以通过。
- 断开状态（Open）：当服务请求失败率达到一定阈值（比如超时、500错误等），熔断器进入断开状态，拒绝对该服务的所有请求，短时间内不再尝试调用该服务。
...
如果项目采用 Sentinel，它的机制主要靠**控制台动态配置 + 注解实现业务降级**。

**熔断体现方式**是：
 我们在 Sentinel 控制台给链路设置阈值，比如异常比率、慢调用比例、异常数量等。

一旦指标超限，Sentinel 会打开断路器，走降级分支。

**降级体现方式**是：

我们在业务方法上用 `@SentinelResource` 标注资源，并通过 `blockHandler` 指定降级方法，Sentinel 触发时会自动调用该降级逻辑，实现快速响应
...
![img](./assets/1764656485115-1-1764724734437-1.png)

![img](./assets/1764656487661-4-1764724734438-2.png)
...
如果项目使用 Hystrix：“海-斯-特瑞克斯”/ˈhɪstrɪks/

**熔断体现**：
 当某接口因调用失败或超时，Hystrix 会检测到高失败率，然后打开熔断器，阻止继续调用坏链路。

**降级体现：**

我们在方法上加 `@HystrixCommand` 并指定 fallbackMethod，一旦熔断被触发，Hystrix 会调用这个降级方法返回兜底响应。

Hystrix 熔断阈值也支持配置，比如：
...
Hystrix 靠 `@HystrixCommand + fallbackMethod` 实现降级，同时靠阈值控制熔断行为。
...
- **分布式调度能力**：支持多机器执行任务，实现扩展和高可用
- **故障容错**：节点挂了不影响任务执行
- **可视化管理**：任务启动、日志查看、调度配置全靠界面操作
- **任务分片**：大任务拆小并行执行，提高效率
- **自动重试**：失败自动补偿
- **任务优先级**：保证关键任务资源优先分配
- **实时监控与告警**

企业级任务调度系统，稳定、可视化、可扩展、可容错。
...
我们项目里 XXL-JOB 用得很广，几乎所有需要周期调度或后台扫描的业务都用它：

例如：

- 门户服务里，通过定时任务刷新区域缓存，作为缓存一致性的兜底方案。
- 营销服务里，我们依赖 JOB 预热优惠券活动，以及自动变更活动状态。
- 支付服务里，用 JOB 异步扫支付结果，再通知业务系统。
- 订单服务里，定时扫描抢单超时订单，回收进入派单池。

一句话概括：
 **凡是需要定时处理、周期扫描、异步任务，我们全用 XXL-JOB。**

===============

我们项目中用xxl-job挺多的，比如：

- 门户缓存的定时更新，使用xxl-job每天凌晨 已启用的区域列表缓存，作为缓存一致性的兜底方案
- 营销服务里，优惠券活动的预热，优惠券活动状态的自动变更
- 支付服务里，定时扫描支付结果并通知给业务服务
- 定时扫描 抢单超时的订单，将超时订单迁入派单池